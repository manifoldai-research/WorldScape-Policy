# Vendored Wan2.2 implementation with optional persistent-context conditioning;
# see docs/provenance.md.
from typing import Any, TypeAlias
import logging

from worldscape_policy.wam.wan22.lctx_attention import AttentionModule
from worldscape_policy.wam.wan22.blocks import (
    WanRMSNorm,
    rope_action_apply,
    WanLayerNorm,
...
from worldscape_policy.wam.action_adapters import (
...
class CausalWanSelfAttention(nn.Module):
    ...
    def _process_clean_image_only(
        ...
        if num_blocks == 0:
            ...
            first_k = clean_image_k[:, :self.frame_seqlen]
            first_v = clean_image_v[:, :self.frame_seqlen]
            if clean_ctx_k is not None and clean_ctx_v is not None:
                first_k = torch.cat([clean_ctx_k, first_k], dim=1)
                first_v = torch.cat([clean_ctx_v, first_v], dim=1)
            return self.attn(
        ...
        if self.local_attn_size == -1 and clean_ctx_k is None:
            ...
            for block_idx in range(num_blocks):
                ...
                # Context: full clean_ctx + first frame + recent clean blocks.
                if self.local_attn_size == -1:
                    image_kv_start = self.frame_seqlen
                else:
                    image_kv_start = max(self.frame_seqlen, block_end - self.local_attn_size * self.frame_seqlen)
                k_parts = []
                v_parts = []
                if clean_ctx_k is not None and clean_ctx_v is not None:
                    k_parts.append(clean_ctx_k)
                    v_parts.append(clean_ctx_v)
                k_parts.extend([
    ...
    def _process_noisy_image_blocks(self, noisy_image_q, noisy_image_k, noisy_image_v,
                                     ...
                                     clean_ctx_k=None, clean_ctx_v=None):
        ...
        first_k = noisy_image_k[:, :self.frame_seqlen]
        first_v = noisy_image_v[:, :self.frame_seqlen]
        if clean_ctx_k is not None and clean_ctx_v is not None:
            first_k = torch.cat([clean_ctx_k, first_k], dim=1)
            first_v = torch.cat([clean_ctx_v, first_v], dim=1)
        output[:, :self.frame_seqlen] = self.attn(
    ...
    def forward(
        ...
        is_tf: bool = True,
        clean_ctx_len: int = 0,
        clean_ctx_freqs: torch.Tensor | None = None,
        permanent_ctx_len: int = 0,
        ...
        if kv_cache is None:
            if is_tf:
                ...
                action_len = action_register_length if action_register_length is not None else 0
                image_pair_len = s - clean_ctx_len - action_len
                if image_pair_len < 0 or image_pair_len % 2 != 0:
                    raise ValueError(
                        "Invalid teacher-forcing sequence layout. Expected "
                        "[clean_ctx][clean_img][noisy_img][action/state]. "
                        f"Got total={s}, clean_ctx_len={clean_ctx_len}, action_register_length={action_len}."
                    )
                half_seq_len = image_pair_len // 2
                clean_start = clean_ctx_len
                noisy_start = clean_start + half_seq_len

                q_clean_ctx = q[:, :clean_ctx_len]
                k_clean_ctx = k[:, :clean_ctx_len]
                if action_register_length is not None:
                    q_context = q[:, clean_start:noisy_start]
                    k_context = k[:, clean_start:noisy_start]
                    q_noisy = q[:, noisy_start:]  
                    k_noisy = k[:, noisy_start:]
                else:
                    q_context = q[:, clean_start:noisy_start]
                    k_context = k[:, clean_start:noisy_start]
                    q_noisy = q[:, noisy_start:]
                ...
                if clean_ctx_len > 0:
                    if clean_ctx_freqs is None:
                        if freqs.shape[0] < clean_ctx_len:
                            raise ValueError(
                                "clean_ctx RoPE frequency length mismatch: "
                                f"clean_ctx_len={clean_ctx_len}, freqs_len={freqs.shape[0]}. "
                                "Pass clean_ctx_freqs created from the context latent grid."
                            )
                        clean_ctx_freqs = freqs[:clean_ctx_len]
                    rq_clean_ctx = rope_action_apply(
                        x=q_clean_ctx,
                        freqs=clean_ctx_freqs,
                        freqs_action=freqs_action,
                        freqs_state=freqs_state,
                        action_register_length=None,
                    ).type_as(v)
                    rk_clean_ctx = rope_action_apply(
                        x=k_clean_ctx,
                        freqs=clean_ctx_freqs,
                        freqs_action=freqs_action,
                        freqs_state=freqs_state,
                    ...
                    roped_query.append(rq_clean_ctx)
                    roped_key.append(rk_clean_ctx)
                ...
                if action_register_length is not None:
                    ...
                    expected_seq_len = clean_ctx_len + half_seq_len + noisy_image_seq_len + action_horizon + state_horizon
                    if roped_query.shape[1] != expected_seq_len:
                        raise ValueError(
                            "Sequence length does not match block layout. "
                            "For 5B use 320x176 (for example, set image_resolution_width=320 and image_resolution_height=176). "
                            f"Got noisy_frames={noisy_frames}, num_image_blocks={num_image_blocks}, "
                            f"clean_ctx_len={clean_ctx_len}, action_register_length={action_register_length}. "
                    ...
                    clean_ctx_q = roped_query[:, :clean_ctx_len]
                    clean_ctx_k = roped_key[:, :clean_ctx_len]
                    clean_ctx_v = v[:, :clean_ctx_len]

                    clean_image_start = clean_ctx_len
                    clean_image_end = clean_image_start + clean_image_seq_len
                    noisy_image_start = clean_image_end
                    noisy_image_end = noisy_image_start + noisy_image_seq_len
                    noisy_action_start = noisy_image_end
                    noisy_action_end = noisy_action_start + action_horizon

                    clean_image_q = roped_query[:, clean_image_start:clean_image_end]
                    clean_image_k = roped_key[:, clean_image_start:clean_image_end]
                    clean_image_v = v[:, clean_image_start:clean_image_end]

                    # Noisy: [image tokens][action tokens][state tokens]
                    noisy_image_q = roped_query[:, noisy_image_start:noisy_image_end]
                    noisy_action_q = roped_query[:, noisy_action_start:noisy_action_end]
                    noisy_state_q = roped_query[:, noisy_action_end:]

                    noisy_image_k = roped_key[:, noisy_image_start:noisy_image_end]
                    noisy_action_k = roped_key[:, noisy_action_start:noisy_action_end]
                    noisy_state_k = roped_key[:, noisy_action_end:]

                    noisy_image_v = v[:, noisy_image_start:noisy_image_end]
                    noisy_action_v = v[:, noisy_action_start:noisy_action_end]
                    noisy_state_v = v[:, noisy_action_end:]

                    clean_ctx_outputs = self.attn(clean_ctx_q, clean_ctx_k, clean_ctx_v) if clean_ctx_len > 0 else clean_ctx_q
                    ...
                    clean_image_outputs = self._process_clean_image_only(
                        ...
                        clean_ctx_k=clean_ctx_k if clean_ctx_len > 0 else None,
                        clean_ctx_v=clean_ctx_v if clean_ctx_len > 0 else None,
                    )
                    ...
                    output_parts = []
                    if clean_ctx_len > 0:
                        output_parts.append(clean_ctx_outputs)
                    output_parts.extend([
                        clean_image_outputs,
                        noisy_image_outputs, noisy_action_outputs, noisy_state_outputs
                    ])
                    x = torch.cat(output_parts, dim=1)
                else:
                    ...
                    clean_start = clean_ctx_len
                    noisy_start = clean_start + half_seq_len
                    clean_q = roped_query[:, clean_start:noisy_start]
                    clean_k = roped_key[:, clean_start:noisy_start]
                    clean_v = v[:, clean_start:noisy_start]
                    noisy_q = roped_query[:, noisy_start:]
                    noisy_k = roped_key[:, noisy_start:]
                    noisy_v = v[:, noisy_start:]
                    
                    # Process clean frames with blockwise causal attention.
                    x_clean = self._process_clean_image_only(
                        clean_q,
                        clean_k,
                        clean_v,
                        half_frames,
                    ...
                    if clean_ctx_len > 0:
                        clean_ctx_outputs = self.attn(clean_ctx_q, clean_ctx_k, clean_ctx_v)
                    
                    # Process noisy frames: attend to all clean frames + themselves
                    full_k_parts = []
                    full_v_parts = []
                    if clean_ctx_len > 0:
                        full_k_parts.append(clean_ctx_k)
                        full_v_parts.append(clean_ctx_v)
                    full_k_parts.extend([clean_k, noisy_k])
                    full_v_parts.extend([clean_v, noisy_v])
                    full_k = torch.cat(full_k_parts, dim=1)
                    full_v = torch.cat(full_v_parts, dim=1)
                    x_noisy = self.attn(noisy_q, full_k, full_v)
                    
                    if clean_ctx_len > 0:
                        x = torch.cat([clean_ctx_outputs, x_clean, x_noisy], dim=1)
                    else:
                ...
                q_clean_ctx = q[:, :clean_ctx_len]
                k_clean_ctx = k[:, :clean_ctx_len]
                q_main = q[:, clean_ctx_len:]
                k_main = k[:, clean_ctx_len:]
                ...
                if clean_ctx_len > 0:
                    roped_query = torch.cat([rq_clean_ctx, rq_main], dim=1)
                    roped_key = torch.cat([rk_clean_ctx, rk_main], dim=1)
                else:
                    roped_query = rq_main
                    roped_key = rk_main
            ...
            q_clean_ctx = q[:, :clean_ctx_len]
            k_clean_ctx = k[:, :clean_ctx_len]
            v_clean_ctx = v[:, :clean_ctx_len]
            q_main = q[:, clean_ctx_len:]
            k_main = k[:, clean_ctx_len:]
            v_main = v[:, clean_ctx_len:]
            ...
            if clean_ctx_len > 0:
                roped_query = torch.cat([rq_clean_ctx, roped_query], dim=1)
                roped_key = torch.cat([rk_clean_ctx, roped_key], dim=1)
                v = torch.cat([v_clean_ctx, v_main], dim=1)
            else:
                v = v_main
            ...
            # Keep the deployment context as a permanent KV prefix; only roll the non-context tail.
            if permanent_ctx_len > 0:
                prefix_len = min(permanent_ctx_len, new_k.shape[1])
                ctx_k, tail_k = new_k[:, :prefix_len], new_k[:, prefix_len:]
                ctx_v, tail_v = new_v[:, :prefix_len], new_v[:, prefix_len:]
                new_k = torch.cat([ctx_k, tail_k[:, -self.max_attention_size:]], dim=1)
                new_v = torch.cat([ctx_v, tail_v[:, -self.max_attention_size:]], dim=1)
            else:
            ...
            if action_register_length is not None:
                action_length = (
                    action_register_length - self.num_state_per_block
                )
                action_query = roped_action_query[:, :action_length]
                action_key = roped_action_key[:, :action_length]
                action_value = action_v[:, :action_length]
                state_query = roped_action_query[:, action_length:]
                state_key = roped_action_key[:, action_length:]
                state_value = action_v[:, action_length:]

                # Match teacher forcing exactly: image and action tokens attend
                # to the cached clean images plus the complete current block,
                # while state tokens remain self-attention-only. Allowing state
                # tokens to absorb image/action history here contaminates the
                # state condition consumed by actions in every subsequent DiT
                # layer.
                image_action_output = self.attn(
                    torch.cat([roped_query, action_query], dim=1),
                    torch.cat([new_k, action_key, state_key], dim=1),
                    torch.cat([new_v, action_value, state_value], dim=1),
                )
                state_output = self.attn(
                    state_query,
                    state_key,
                    state_value,
                )
                x = torch.cat([image_action_output, state_output], dim=1)
            else:
                x = self.attn(
                    roped_query,
                    new_k,
...
class CausalWanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        y, updated_kv_cache = self.self_attn(
            ...
            clean_ctx_len=clean_ctx_len,
            clean_ctx_freqs=clean_ctx_freqs,
            permanent_ctx_len=permanent_ctx_len,
        )
        x = x + (y * e[2].squeeze(2))

        # cross-attention & ffn function
        def cross_attn_ffn(x, context, e, num_img_tokens):
            x = x + self.cross_attn(self.norm3(x), context, num_img_tokens=num_img_tokens)
            y = self.ffn(
        ...
        x = cross_attn_ffn(x, context, e, num_img_tokens)
        return x, updated_kv_cache
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    def __init__(self,
        ...
        if kwargs:
            LOGGER.warning(
                "CausalWanModel ignored unknown init kwargs: %s",
                sorted(kwargs.keys()),
            )
        ...
        self._wan22_distributed_context = None
    ...
    def set_image_parallel_context(self, context: Any | None) -> None:
        """Attach non-owning distributed execution hooks.

        The context is runtime state, not a module or checkpoint-owned object.
        """

        self._wan22_distributed_context = context
    ...
    def _resolve_embodiment_ids(
        ...
        *,
        ...
        """Resolve pretrain category rows or the sole posttrain-local row."""

        if self.max_num_embodiments == 1:
            return torch.zeros(batch_size, dtype=torch.long, device=device)
        if embodiment_id is None:
            raise ValueError("Multi-adapter WAM requires embodiment_id")
        resolved = embodiment_id.to(device=device, dtype=torch.long)
        if resolved.ndim == 0:
            resolved = resolved.repeat(batch_size)
        if resolved.shape != (batch_size,):
            raise ValueError(
                "embodiment_id must have shape "
                f"[{batch_size}], got {tuple(resolved.shape)}"
            )
        if bool((resolved < 0).any()) or bool(
            (resolved >= self.max_num_embodiments).any()
        ):
            raise ValueError(
                "embodiment_id is outside configured adapter categories "
                f"[0, {self.max_num_embodiments})"
            )
        return resolved
    ...
    def _forward_blocks(
        ...
        clean_ctx: torch.Tensor | None = None,
        clean_ctx_freqs: torch.Tensor | None = None,
        num_img_tokens: int = 0,
        permanent_ctx_len: int = 0,
        ...
        if clean_ctx is not None:
            clean_ctx_len = clean_ctx.shape[1]
            x = torch.cat([clean_ctx, x], dim=1)

        if action is not None:
            embodiment_id = self._resolve_embodiment_ids(
                embodiment_id,
                batch_size=B,
                device=x.device,
            )
        ...
        if clean_ctx_len > 0:
            timestep_ctx = torch.zeros(
                B,
                clean_ctx_len,
                device=timestep.device,
                dtype=timestep.dtype,
            )
            e_clean_ctx = self.time_embedding(
                sinusoidal_embedding_1d(self.freq_dim, timestep_ctx.flatten()).type_as(x))
            e_clean_ctx = e_clean_ctx.unflatten(dim=0, sizes=(B, clean_ctx_len))
            e0_clean_ctx = self.time_projection(e_clean_ctx)
            e0_clean_ctx = e0_clean_ctx.unflatten(dim=2, sizes=(6, self.dim))
            e0 = torch.cat([e0_clean_ctx, e0], dim=1)
        ...
        if clip_feature is not None:
            num_img_tokens = clip_feature.shape[1]
        ...
        for block_index, block in enumerate(self.blocks):
            def run_block(
                hidden_states,
                current_block=block,
                current_kv_cache=kv_cache[block_index],
            ):
            ...
            distributed = self._wan22_distributed_context
            image_parallel = (
                getattr(distributed, "image_parallel", None)
                if distributed is not None
                else None
            )
            if image_parallel is None:
                x, updated_kv_cache = run_block(x)
            else:
                x, updated_kv_cache = image_parallel.run_block(
    ...
    def _forward_inference(
        ...
        clean_ctx_freqs = None
        include_clean_ctx = (
            clean_ctx is not None
            and kv_cache is not None
            and len(kv_cache) > 0
            and kv_cache[0].shape[2] == 0
        )
        if include_clean_ctx:
            clean_ctx = self.patch_embedding(clean_ctx)
            clean_ctx_grid_size = torch.tensor(clean_ctx.shape[2:], dtype=torch.long)
            clean_ctx_freqs = self._create_freqs(
                grid_size=clean_ctx_grid_size,
                start_frame=0,
            )
            clean_ctx = clean_ctx.flatten(start_dim=2).transpose(1, 2)
        else:
            clean_ctx = None
    ...
    def _forward_train(
        ...
        if clean_ctx is not None:
            ...
            if aug_t is None:
                aug_t_ctx = torch.zeros(
                    B,
                    clean_ctx_len,
                    device=timestep_original.device,
                    dtype=timestep_original.dtype,
                )
            elif aug_t.shape[1] == clean_ctx_len:
                aug_t_ctx = aug_t
            elif aug_t.shape[1] == 1:
                aug_t_ctx = aug_t.expand(B, clean_ctx_len)
            else:
                ctx_indices = torch.linspace(
                    0,
                    aug_t.shape[1] - 1,
                    clean_ctx_len,
                    device=aug_t.device,
                ...
                aug_t_ctx = aug_t[:, ctx_indices]

            e_clean_ctx = self.time_embedding(
                sinusoidal_embedding_1d(self.freq_dim, aug_t_ctx.flatten()).type_as(x))
            e_clean_ctx = e_clean_ctx.unflatten(dim=0, sizes=(B, clean_ctx_len))
            e0_clean_ctx = self.time_projection(e_clean_ctx)
            e0_clean_ctx = e0_clean_ctx.unflatten(dim=2, sizes=(6, self.dim))
            e0 = torch.cat([e0_clean_ctx, e0], dim=1)
        ...
        kwargs = dict(
            ...
            clean_ctx_len=clean_ctx.shape[1] if clean_ctx is not None else 0,
            clean_ctx_freqs=clean_ctx_freqs,
            num_img_tokens=num_img_tokens,
        )
        ...
        prefix_len = 0
        if clean_ctx is not None:
            prefix_len += clean_ctx.shape[1]
        if clean_x is not None:
            prefix_len += clean_x.shape[1]
        if prefix_len:
            x = x[:, prefix_len:]
