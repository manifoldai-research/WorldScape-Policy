...
try:
    ...
    # cuRobo 0.7.x uses the former public `warp.torch` namespace. Warp 1.14
    # moved that integration module under `warp._src`, so expose a local
    # compatibility alias without changing the shared Python environment.
    import importlib
    import warp as wp

    if not hasattr(wp, "torch"):
        wp.torch = importlib.import_module("warp._src.torch")
