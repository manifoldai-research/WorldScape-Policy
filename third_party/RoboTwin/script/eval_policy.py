...
def main(usr_args, model=None):
    ...
    max_seed_trials = usr_args.get("max_seed_trials")
    if max_seed_trials is not None:
        max_seed_trials = int(max_seed_trials)
        if max_seed_trials <= 0:
            raise ValueError(
                f"`max_seed_trials` must be > 0, got: {max_seed_trials}"
            )
...
def eval_policy(task_name,
    ...
    while succ_seed < test_num:
        if max_seed_trials is not None and seed_trials >= max_seed_trials:
            print(
                f"Reached `max_seed_trials`={max_seed_trials} before collecting "
                f"{test_num} accepted seeds. accepted={succ_seed}, "
                f"current_seed={now_seed}."
            )
            break
        seed_trials += 1
        ...
        while TASK_ENV.take_action_cnt < TASK_ENV.step_lim:
            manages_chunks = bool(
                getattr(model, "manages_action_chunks", False)
            )
            need_obs = not manages_chunks
            if (
                need_obs
        ...
        if hasattr(model, "record_episode_result"):
            episode_summary = model.record_episode_result(
                success=succ,
                task_name=args["task_name"],
                task_config=args["task_config"],
                seed=now_seed,
                episode_index=TASK_ENV.test_num,
                take_action_count=TASK_ENV.take_action_cnt,
                step_limit=TASK_ENV.step_lim,
                instruction=instruction,
            )
            timing = episode_summary.get("timing") or {}
            print(
                "[robotwin_timing] "
                f"episode={episode_summary['episode_index']} "
                f"success={episode_summary['success']} "
                f"infer_count={timing.get('infer_count')} "
                f"infer_s_mean={timing.get('infer_s_mean')} "
                f"infer_s_total={timing.get('infer_s')} "
                f"sim_s_total={timing.get('sim_s')}"
            )
